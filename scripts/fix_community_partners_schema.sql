-- =====================================================
-- FIX COMMUNITY PARTNERS SCHEMA FOR PRODUCTION
-- =====================================================
-- Run this script in Supabase SQL Editor to add missing columns
-- and create missing tables for Community Sales Partner functionality.
-- =====================================================
...
-- 2. Add missing columns to community_partners table
-- (Each ADD COLUMN IF NOT EXISTS to avoid errors if column already exists)
...
ALTER TABLE community_partners ADD COLUMN IF NOT EXISTS kyc_verified_by UUID REFERENCES users(id) ON DELETE SET NULL;
...
-- Tier relationship
ALTER TABLE community_partners ADD COLUMN IF NOT EXISTS tier_id UUID REFERENCES partner_tiers(id) ON DELETE SET NULL;

-- 3. Create partner_commissions table if not exists
...
-- 4. Create partner_payouts table if not exists
...
-- Add foreign key for payout_id in partner_commissions (now that partner_payouts exists)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints
        WHERE constraint_name = 'partner_commissions_payout_id_fkey'
    ) THEN
...
-- 5. Create partner_referrals table if not exists
...
-- 6. Create partner_training table if not exists
...
-- 7. Create partner_orders table if not exists
...
-- 9. Insert default partner tiers (if not exist)
...
-- 10. Verify the fix
SELECT 'community_partners columns:' as info;
SELECT column_name, data_type
FROM information_schema.columns
WHERE table_name = 'community_partners'
ORDER BY ordinal_position;

SELECT 'partner_tiers:' as info;
SELECT code, name, commission_percentage, is_default FROM partner_tiers;
